...
struct SHDRSectionHeader {
  u32 magic;       // "SHDR" = 0x53484452 (big-endian ASCII) or 0x52444853 (LE)
  u32 section_len; // Length of section data following this header (big-endian)
  u16 name_len;    // Length of name string (big-endian)
  u16 version_len; // Length of version string (big-endian)
  ...
  u32 checksum;    // Section checksum (big-endian)
  u8 reserved[4];  // Reserved (to make 64 bytes total per SHDR_SIZE)
  // Total: 4 + 4 + 2 + 2 + 1 + 3 + 32 + 8 + 4 + 4 = 64 bytes
};
